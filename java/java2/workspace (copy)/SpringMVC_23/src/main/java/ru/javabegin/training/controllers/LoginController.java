...
public class LoginController {
	...
	@RequestMapping(value = "/checkStrength", method = RequestMethod.GET, produces = { "text/html; charset=UTF-8" })  // js func for checking password
	public @ResponseBody
	...
	public String main(@ModelAttribute User user, HttpSession session, Locale locale) {          // mainpage or login again
		...
		return "login";          // MESSAGE SOURCE = CURRENT LOCALE IS + {0} = EN,
	}

	
	// PROBLEM WITH REDIRECT + INTERCEPTOR!
	...
	public ModelAndView checkUser(Locale locale, @Valid @ModelAttribute("user") User user, BindingResult bindingResult, ModelMap modelMap, RedirectAttributes redirectAttributes) {
                                                 // what attrib is valid
		ModelAndView modelAndView = new ModelAndView();
		
//		model.addAttribute("user", user);
		System.out.println(user.getName()+ "LOGINCONTROLLER||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
		modelMap.addAttribute("user", user);
		modelAndView.addAllObjects(modelMap);
//		modelAndView.getModel().
		if (!bindingResult.hasErrors()) {
			System.out.println(user.getName()+ "LOGINCONTROLLER2||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
//			RedirectView redirectView = new RedirectView("mainpage");
//			redirectView.setStatusCode(HttpStatus.MOVED_PERMANENTLY);
//			modelAndView.setView(redirectView);
			modelAndView.setViewName("main");
               // some data about locale
			System.out.println(user.getName()+ "LOGINCONTROLLER3||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
//			redirectAttributes.addFlashAttribute("locale", messageSource.getMessage("locale", new String[] { locale.getDisplayName(locale) }, locale));
			...
			System.out.println(user.getName()+ "LOGINCONTROLLER4||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
			modelAndView.setViewName("login");
//			redirectAttributes.addFlashAttribute("locale", messageSource.getMessage("locale", new String[] { locale.getDisplayName(locale) }, locale));
		}
		System.out.println(user.getName()+ "LOGINCONTROLLER5||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||");
		return modelAndView;
	}
	
//	@RequestMapping(value = "/check-user", method = RequestMethod.POST)  // 10 mvc
...
//	}
	...
	@RequestMapping(value = "/mainpage", method = RequestMethod.GET) // go to the main page
	...
	@RequestMapping(value = "/downloadPDF", method = RequestMethod.GET)   // go to the pdfView
	public ModelAndView downloadPDF() {

		return new ModelAndView("pdfView");
	}

	@RequestMapping(value = "/failed", method = RequestMethod.GET)      // go to login-failed with message
	public ModelAndView failed() {
		return new ModelAndView("login-failed", "message", "Login failed!"); // message is the login-failed`s field
	}

	@RequestMapping(value = "/get-json-user/{name}/{admin}", method = RequestMethod.GET, produces = "application/json")  // get json
	@ResponseBody             //INCLUDE NAME IN URL
	...
	public ResponseEntity<String> setJsonUser(@RequestBody User user) {
		...
		return new ResponseEntity<String>(HttpStatus.OK);     // STATUS OF RESPONSE
	}
